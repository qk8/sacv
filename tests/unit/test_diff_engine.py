...
class TestValidateNoFullOverwrite:
    ...
    def test_targeted_diff_passes(self, tmp_path):
        ...
        errors = engine.validate_no_full_overwrite([diff])
        assert errors == []

    def test_full_overwrite_rejected(self, tmp_path):
    ...
    def test_create_operation_allowed(self, tmp_path):
    ...
    def test_delete_operation_allowed(self, tmp_path):
    ...
    def test_nonexistent_file_treated_as_create(self, tmp_path):
    ...
    def test_empty_file_treated_as_create(self, tmp_path):
    ...
    def test_boundary_at_exact_ratio(self, tmp_path):
    ...
    def test_below_boundary_passes(self, tmp_path):
    ...
    def test_multiple_diffs_independent_validation(self, tmp_path):
        ...
        errors = engine.validate_no_full_overwrite(diffs)
        assert len(errors) == 1
        assert errors[0].file_path == "Bad.java"

    def test_added_lines_not_counted_as_removal(self, tmp_path):
...
class TestApplyDiffs:
    ...
    def test_delete_nonexistent_file_succeeds(self, tmp_path):
    ...
    def test_patch_failure_creates_conflict(self, tmp_path):
    ...
    def test_apply_diffs_uses_asyncio_to_thread(self, tmp_path):
...
class TestGenerateAstDiff:
    ...
    def test_produces_unified_diff(self):
        engine = DiffEngine(".")
        diff = engine.generate_ast_diff("line1\nline2\n", "line1\nline3\n")
    ...
    def test_create_operation_diff(self):
        engine = DiffEngine(".")
        diff = engine.generate_ast_diff("", "new content")
        assert diff.operation == "modify"
        assert "+new content" in diff.diff_content

    def test_delete_operation_diff(self):
        engine = DiffEngine(".")
        diff = engine.generate_ast_diff("old content", "")
        assert diff.operation == "modify"
